...
const app = express();
app.use(express.static("../lucky_strick_eas_client/build"));
app.use(cors());
...
// const MongoClient = require("mongodb").MongoClient;
const uri = "mongodb://127.0.0.1:27017/luckyStrick_EAS";
// const uri =
//   "mongodb+srv://aktcl:01939773554op5t@cluster0.9akoo.mongodb.net/luckyStrick_EAS?retryWrites=true&w=majority";
const client = new MongoClient(uri, {
  useNewUrlParser: true,
  useUnifiedTopology: true,
});
client.connect((err) => {
  const userCollection = client.db("luckyStrick_EAS").collection("users");
  const adminCollection = client.db("luckyStrick_EAS").collection("admins");
  const leadsCollection = client.db("luckyStrick_EAS").collection("leads");
  ...
  app.get("*", (req, res) => {
    res.sendFile(
      path.join(__dirname, "../lucky_strick_eas_client/build", "index.html")
    );
  });
});
